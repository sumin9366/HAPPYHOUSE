    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div v-if="$store.state.boardStore.fileList.length > 0" class="mb-3">
            첨부파일 :
            <span
              ><div
                v-for="(file, index) in $store.state.boardStore.fileList"
                class="fileName"
                :key="index"
              >
                {{ file.fileName }}
...
export default {
  ...
  // v-model -title
  computed: {
    // v-model="storeTitle" 이름 그대로 사용하면 get()과 set()이 알아서 호출됨
    // 다른 component와의 충돌을 막기 위해 store를 사용함
    storeTitle: {
      get() {
        return this.$store.state.boardStore.boardTitle;
      },
      set(title) {
  ...
  methods: {
    // modal 초기화
    initUI() {
      this.CKEditor.setData(this.$store.state.boardStore.boardContent);
    ...
    async boardUpdate() {
      ...
      formData.append("boardId", this.$store.state.boardStore.boardId); // update 에 추가
      formData.append("boardTitle", this.$store.state.boardStore.boardTitle);
      formData.append("boardContent", this.CKEditor.getData()); // store X !!!!
      ...
      try {
        let { data } = await http.post(
          "/boards/" + this.$store.state.boardStore.boardId,
          formData,
          options
        );
